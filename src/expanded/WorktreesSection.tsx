...
function NotesEditor({ path, notes, variant, onSaved }: {
    ...
    return (
      <div>
        ...
        <textarea
          ...
          className="w-full bg-[var(--c-surface-2)] border border-[var(--c-accent)]/40 rounded-lg px-2.5 py-1.5 text-[12.5px] text-[var(--c-text)] outline-none resize-y leading-relaxed"
        />
        {notes && (
          <button
            onMouseDown={e => e.preventDefault()}
            onClick={deleteNote}
            className="text-[10.5px] text-[var(--c-text-3)] hover:text-rose-400 transition-colors mt-1"
          >
            Delete note
          </button>
        )}
    ...
    return (
      <div className="w-full flex items-start gap-1.5 rounded-lg border border-[var(--c-border)]/60 bg-[var(--c-surface-2)]/40 px-2.5 py-1.5 hover:border-[var(--c-text-3)]/40 transition-colors">
        ...
        >
          ...
          <span className="text-[12.5px] text-[var(--c-text-2)] whitespace-pre-wrap leading-relaxed line-clamp-3 align-middle">{notes}</span>
        </button>
        <button
          onClick={deleteNote}
          title="Delete note"
          aria-label="Delete note"
          className="flex-shrink-0 text-[var(--c-text-3)] hover:text-rose-400 transition-colors text-[12px] mt-0.5"
        >
          ✕
        </button>
      </div>
  ...
  return (
    <button
      ...
      className="text-[11.5px] text-[var(--c-text-3)] hover:text-[var(--c-text-2)] transition-colors"
    >
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-6">
        ...
        {counts.safe > 0 && (
          <div className="flex items-center justify-between gap-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-2.5 mb-4">
            ...
            <button
              ...
              className="text-[12px] px-2.5 py-1 rounded-md bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 transition-colors font-medium"
            >
              Review
            </button>
          </div>
        ...
        {!loading && visibleRepos.length === 0 && (
          <p className="text-[13px] text-[var(--c-text-3)] text-center py-10">
            {allWts.length === 0
        ...
        {visibleRepos.map(({ repo, items }, repoIndex) => {
          ...
          <div key={repo.repoPath} className="mb-3 rounded-xl border border-[var(--c-border)] bg-[var(--c-surface-2)]/25 overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-3.5 py-2.5">
              ...
              >
                <span
                  className="w-7 h-7 rounded-lg flex items-center justify-center font-mono font-bold text-[13px] text-black/80 shrink-0"
                ...
                <span className="min-w-0">
                  {renaming === repo.repoPath ? (
                    <input
                      ...
                      maxLength={80}
                      className="block w-48 bg-[var(--c-surface-2)] border border-[var(--c-accent)]/40 rounded px-1.5 py-0.5 text-[14px] font-semibold text-[var(--c-text)] outline-none"
                    />
                  ) : (
                    <span className="block text-[14.5px] font-semibold truncate group-hover/repo:text-[var(--c-accent)] transition-colors">
                      {displayName(repo)}
                      {repoNames[repo.repoPath] && (
                        <span className="ml-1.5 text-[11px] font-normal font-mono text-[var(--c-text-3)]">({repo.repoName})</span>
                      )}
                    </span>
                  )}
                  <span className="block text-[11.5px] text-[var(--c-text-3)]">
                    {items.length} worktree{items.length > 1 ? 's' : ''} · base {repo.baseBranch}
                  </span>
                  <span className="block text-[11px] font-mono text-[var(--c-text-3)]/70 truncate" title={repo.repoPath}>
                    {repo.repoPath}
                  </span>
                </span>
              </button>
              <div className="flex items-center gap-1.5 shrink-0">
                <button
                  ...
                  className="text-[11px] px-1.5 py-1 rounded-md text-[var(--c-text-3)] hover:text-[var(--c-text-2)] transition-colors"
                >
                  ✎
                </button>
                <button
                  ...
                  className="flex items-center gap-1 text-[11px] px-2.5 py-1 rounded-md border border-[var(--c-border)] text-[var(--c-text-3)] hover:text-[var(--c-text-2)] hover:border-[var(--c-text-3)]/50 transition-colors"
                >
                  ◷ Sessions
                </button>
                <button
                  ...
                  className={`flex items-center gap-1 text-[11px] px-2.5 py-1 rounded-md border transition-colors ${insightsOpen[repo.repoPath] ? 'border-[var(--c-accent)]/50 bg-[var(--c-accent)]/10 text-[var(--c-accent)]' : 'border-[var(--c-border)] text-[var(--c-text-3)] hover:text-[var(--c-text-2)] hover:border-[var(--c-text-3)]/50'}`}
                >
                ...
                {vscodeAvailable && (
                  <button
                    ...
                    className="text-[11px] px-2.5 py-1 rounded-md border border-[var(--c-border)] text-[var(--c-text-3)] hover:text-[var(--c-text-2)] hover:border-[var(--c-text-3)]/50 transition-colors"
                  >
                    VS Code
                  </button>
                )}
                <button
                  ...
                  className={`text-[var(--c-text-3)] text-[13px] px-1 transition-transform ${open ? 'rotate-90' : ''}`}
                >
                  ›
                </button>
              </div>
                    ...
                    >
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[14px] font-mono font-semibold truncate">
                            {wt.branch ?? (wt.isDetached ? 'detached HEAD' : '?')}
                          </span>
                        </div>
                        <div className="text-[12px] text-[var(--c-text-3)] mt-0.5 ml-4">
                          {relativeTime(wt.lastCommitTs)}
                          {linked.length > 0 && <> · {linked.length} session{linked.length > 1 ? 's' : ''}</>}
                        </div>
                        <div className="text-[11px] font-mono text-[var(--c-text-3)]/70 mt-0.5 ml-4 truncate" title={wt.path}>
                          {wt.path}
                        </div>
                      </div>
                      <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        <span className={`text-[var(--c-text-3)] text-[13px] transition-transform ${isOpen ? 'rotate-90' : ''}`}>›</span>
                      </div>
                    </button>
                    ...
                    {isOpen && (
                      <div className="px-4 pb-4 border-t border-[var(--c-border)] pt-3">
                        ...
                        {wt.lastCommitSubject && (
                          <p className="text-[12px] font-mono text-[var(--c-text-3)] mb-3 truncate" title={wt.lastCommitSubject}>
                            {wt.lastCommitSubject}
                          </p>
                        )}
                        ...
                        <div className="flex gap-2 mb-3">
                          <button
                            ...
                            className={`text-[12px] px-3 py-1.5 rounded-md font-medium transition-colors ${copied === wt.path ? 'bg-emerald-500/20 text-emerald-400' : 'bg-[var(--c-accent)]/15 text-[var(--c-accent)] hover:bg-[var(--c-accent)]/25'}`}
                          >
                          ...
                          {vscodeAvailable && (
                            <button
                              ...
                              className="text-[12px] px-3 py-1.5 rounded-md border border-[var(--c-border)] text-[var(--c-text-3)] hover:text-[var(--c-text-2)] transition-colors"
                            >
                              VS Code
                            </button>
                          )}
                          ...
                          {isSafeToDelete(wt) && confirmDelete !== wt.path && (
                            <button
                              ...
                              className="text-[12px] px-3 py-1.5 rounded-md border border-[var(--c-border)] text-rose-400/80 hover:text-rose-400 hover:border-rose-400/40 transition-colors"
                            >
                              Delete
                            </button>
                          )}
                          {confirmDelete === wt.path && (
                            <>
                              <button
                                ...
                                className="text-[12px] px-3 py-1.5 rounded-md bg-rose-500/20 text-rose-400 hover:bg-rose-500/30 transition-colors font-medium disabled:opacity-50"
                              >
                        ...
                        {removeError && confirmDelete === wt.path && (
                          <p className="text-[12px] text-rose-400 bg-rose-500/10 rounded-lg px-3 py-2 mb-3">{removeError}</p>
                        )}
                        {linked.length > 0 && (
                          <div className="space-y-1.5">
                            {linked.map(s => (
                              <button
                                ...
                                className="block w-full text-left text-[12px] border-b border-[var(--c-border)]/50 last:border-0 pb-1.5 last:pb-0 hover:text-[var(--c-accent)] transition-colors group/session"
                              >
  ...
  if (data === 'loading') {
    return (
      <div className="flex items-center gap-2 mb-2 px-1">
        ...
        <span className="text-[12px] text-[var(--c-text-3)]">Loading repo insights…</span>
      </div>
    )
  }
  if (data.sessionsAnalyzed === 0) {
    return <p className="text-[12px] text-[var(--c-text-3)] mb-2 px-1">No analyzed sessions in the last 30 days</p>
  }
  return (
    <div className="rounded-xl border border-[var(--c-border)] bg-[var(--c-surface-2)]/40 px-3.5 py-2.5 mb-2">
      ...
      {data.toolCounts.length > 0 && (
        <div className="mt-3 pt-2.5 border-t border-[var(--c-border)]/60">
          <p className="text-[11px] font-mono text-[var(--c-text-3)] uppercase tracking-wider mb-2">Top tools in this repo</p>
...
function Badge({ tone, children }: { tone: 'ok' | 'warn' | 'accent' | 'muted'; children: React.ReactNode }) {
  ...
  return <span className={`text-[11px] font-mono px-2 py-0.5 rounded-full whitespace-nowrap ${cls}`}>{children}</span>
}

function DetailCell({ k, v }: { k: string; v: string }) {
  return (
    <div className="rounded-lg bg-[var(--c-surface-2)] px-3 py-2">
      <div className="text-[13.5px] font-mono font-semibold">{v}</div>

      ...
      >
        <div className="flex items-center gap-2.5 px-4 py-3.5 border-b border-[var(--c-border)]">
          ...
          <input
            ...
            className="flex-1 bg-transparent outline-none text-[15px] placeholder:text-[var(--c-text-3)]"
          />
          <span className="text-[11px] font-mono text-[var(--c-text-3)] border border-[var(--c-border)] rounded px-1.5 py-0.5 flex-shrink-0">
            ESC
          </span>
        </div>
        <div className="max-h-[360px] overflow-y-auto p-1.5">
          {scored.length === 0 && (
            <div className="py-8 text-center text-[13px] text-[var(--c-text-3)]">No matches for "{query}"</div>
          )}
          {[...groups.entries()].map(([group, list]) => (
            <div key={group}>
              <div className="px-2.5 pt-2 pb-1 text-[10.5px] font-mono uppercase tracking-wider text-[var(--c-text-3)]">
                {group}
              </div>
              {list.map(({ item, m }) => {
                flatIndex++
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-left text-[13.5px] transition-colors ${idx === sel ? 'bg-indigo-400/15 text-indigo-400' : 'text-[var(--c-text-2)] hover:bg-[var(--c-hover)]'}`}
                  >
                    <span className="flex-1 min-w-0 truncate">{highlightMatch(item.title, m.idx)}</span>
                    {item.sub && (
                      <span className="text-[11.5px] text-[var(--c-text-3)] flex-shrink-0 max-w-[40%] truncate">
                        {item.sub}
                      </span>
                    )}
                  </button>
        ...
        <div className="flex gap-4 px-3.5 py-2 border-t border-[var(--c-border)] text-[11px] text-[var(--c-text-3)]">

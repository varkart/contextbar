...
describe('McpsListPanel', () => {
  it('renders tool name breadcrumb', () => {
    render(<McpsListPanel tool={tool} onBack={vi.fn()} onSelectMcp={vi.fn()} />)
  ...
  it('no lock icon for MCPs without secrets', () => {
    ...
    render(<McpsListPanel tool={singleTool} onBack={vi.fn()} onSelectMcp={vi.fn()} />)
    expect(screen.queryByLabelText('has env secrets')).not.toBeInTheDocument()
  })

  it('back button calls onBack', () => {
    const onBack = vi.fn()
    render(<McpsListPanel tool={tool} onBack={onBack} onSelectMcp={vi.fn()} />)
  ...
  it('clicking MCP calls onSelectMcp', () => {
    ...
    render(<McpsListPanel tool={tool} onBack={vi.fn()} onSelectMcp={onSelectMcp} />)
  ...
  it('hides search input when 5 or fewer MCPs', () => {
    ...
    render(<McpsListPanel tool={smallTool} onBack={vi.fn()} onSelectMcp={vi.fn()} />)

...
describe('SkillsListPanel', () => {
  it('lists all skills', () => {
    render(<SkillsListPanel agent={agent} onBack={vi.fn()} onSelectSkill={vi.fn()} onAddSkill={vi.fn()} />)
  ...
  it('clicking skill calls onSelectSkill', () => {
    ...
    render(<SkillsListPanel agent={agent} onBack={vi.fn()} onSelectSkill={onSelectSkill} onAddSkill={vi.fn()} />)
    fireEvent.click(screen.getByText('impeccable'))
    expect(onSelectSkill).toHaveBeenCalledWith(agent.skills[0])
  })
  ...
  it('shows search input even with 3 skills', () => {
    const smallTool = { ...agent, skills: agent.skills.slice(0, 3) }
    render(<SkillsListPanel agent={smallTool} onBack={vi.fn()} onSelectSkill={vi.fn()} onAddSkill={vi.fn()} />)
    expect(screen.getByPlaceholderText('Filter skills…')).toBeInTheDocument()
  })

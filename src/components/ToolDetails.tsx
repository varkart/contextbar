...
export default function ToolDetails({ tool, query, matchedSkills, matchedMcps, onSelectSkill, onSelectMcp }: ToolDetailsProps) {
  return (
    <div className="px-2 pt-1 pb-3 space-y-3">
      {tool.error && (
        <p className="text-[11px] text-red-400/80 px-2 font-mono">{tool.error}</p>
      )}
      <SkillSection skills={tool.skills} query={query} matchedPaths={matchedSkills} onSelectSkill={onSelectSkill} />

...
function ThemeSelector({ value, onChange }: { value: ThemePreference; onChange: (t: ThemePreference) => void }) {
  ...
  return (
    <div className="flex gap-0.5 p-1 bg-[var(--c-surface)] rounded-lg border border-[var(--c-border)]">
      {options.map(t => (
        <button
          key={t}
          onClick={() => onChange(t)}
          className={`flex-1 py-0.5 text-[11px] rounded-md capitalize transition-colors ${
            value === t
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        ...
        <div className="divide-y divide-[var(--c-border-sub)]">
          ...
          <SettingRow label="Source">
            <a href="https://github.com/varkart/agentbar" target="_blank" rel="noopener noreferrer"
              className="text-[11px] text-indigo-500 hover:text-indigo-400 transition-colors flex items-center gap-1">
              github.com/varkart/agentbar <ExternalLinkIcon />
            </a>
          </SettingRow>
        </div>
      </div>

...
import { searchAgents, highlight } from '../search'
import type { Agent } from '../types'

const makeTool = (overrides: Partial<Agent> = {}): Agent => ({
  id: 'claude',
  name: 'Claude Code',
  installed: true,
  supportsSkills: true,
...
describe('searchAgents', () => {
  it('empty query returns all tools with score 1', () => {
    ...
    const results = searchAgents(tools, '')
  ...
  it('whitespace-only query returns all tools', () => {
    const tools = [makeTool({ id: 'a' }), makeTool({ id: 'b' })]
    expect(searchAgents(tools, '   ')).toHaveLength(2)
  })

  it('matches tool by name', () => {
    const tools = [makeTool({ name: 'Claude Code' }), makeTool({ id: 'cursor', name: 'Cursor' })]
    const results = searchAgents(tools, 'claude')
    expect(results).toHaveLength(1)
    expect(results[0].agent.name).toBe('Claude Code')
  })
  ...
  it('excludes tools with no match', () => {
    ...
    expect(searchAgents(tools, 'gemini')).toHaveLength(0)
  })
  ...
  it('matches by skill name and records matched path', () => {
    ...
    const results = searchAgents([tool], 'impec')
    expect(results).toHaveLength(1)
  ...
  it('matches by skill description', () => {
    ...
    const results = searchAgents([tool], 'graph')
    expect(results).toHaveLength(1)
  ...
  it('matches by mcp name and records matched name', () => {
    ...
    const results = searchAgents([tool], 'github')
    expect(results).toHaveLength(1)
  ...
  it('sorts by score descending', () => {
    ...
    const results = searchAgents([partial, exact], 'gemini')
    // exact match should score higher than prefix
    expect(results[0].agent.id).toBe('gemini')
  })

  it('case-insensitive matching', () => {
    const tools = [makeTool({ name: 'Claude Code' })]
    expect(searchAgents(tools, 'CLAUDE')).toHaveLength(1)
  })

  it('fuzzy match: all chars in order', () => {
    const tools = [makeTool({ name: 'Claude Code' })]
    // 'cde' appears in order in 'Claude Code' (c...d...e)
    expect(searchAgents(tools, 'cde')).toHaveLength(1)
  })
})

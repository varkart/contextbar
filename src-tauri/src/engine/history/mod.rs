...
/// Path of the JSONL transcript backing a Claude session, verified to
/// exist (falls back to a directory scan if the encoded-path guess misses).
pub fn session_file(
    home: &std::path::Path,
    project: &str,
    session_id: &str,
) -> Option<std::path::PathBuf> {
    index::resolve_session_file(home, project, session_id)
}
